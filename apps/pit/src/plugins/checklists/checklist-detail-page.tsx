...
export function ChecklistDetailPage() {
  ...
  useEffect(() => { load(); }, [id]);
  useEffect(() => { if (editingListField || editingItem) editInputRef.current?.focus(); }, [editingListField, editingItem]);
  ...
  async function saveListName() {
    ...
    setEditingListField(null); setBanner(null);
    await load();
  }
  ...
  async function handleAddItem() {
    if (!list) return;
    if (!newItemName.trim()) { setAddError("Name is required."); return; }
    const res = await api.lists[":id"].items.$post({
      param: { id: String(list.id) },
      json: { name: newItemName.trim(), description: newItemDesc.trim() || null },
    });
    if (!res.ok) { setBanner(await getErrorMessage(res as unknown as Response)); return; }
    setNewItemName(""); setNewItemDesc(""); setAdding(false); setAddError(""); setBanner(null);
    await load();
  }
  ...
  async function saveItemEdit() {
    ...
    setEditingItem(null); setBanner(null);
    await load();
  }
  ...
  if (notFound || !list) {
    return (
      <main className="min-h-screen bg-gray-950 flex flex-col items-center justify-center gap-4">
        ...
        <button type="button" onClick={() => navigate("/editor")} className="text-red-400 hover:text-red-300 text-sm underline">
          Back to Checklists
        </button>
      </main>
    );
  ...
  return (
    <main className="min-h-screen bg-gray-950 text-white">
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
        ...
        <button type="button" onClick={() => navigate("/editor")} className="text-sm text-gray-500 hover:text-gray-300 transition-colors">
          ← Back to Checklists
        </button>
          ...
          {editingListField === "name" ? (
            <div className="flex gap-2">
              <input
                ...
                onKeyDown={(e) => { if (e.key === "Enter") saveListName(); if (e.key === "Escape") setEditingListField(null); }}
              ...
              />
              <button type="button" onClick={saveListName} className="px-4 py-2 bg-red-600 hover:bg-red-500 text-white text-sm font-semibold rounded-lg">Save</button>
              <button type="button" onClick={() => setEditingListField(null)} className="px-3 py-2 bg-gray-700 hover:bg-gray-600 text-gray-200 text-sm rounded-lg">✕</button>
            </div>
          ) : (
            <div className="flex items-center gap-2">
              <h1 className="text-3xl font-bold tracking-tight">{list.name}</h1>
              <button type="button" title="Rename" onClick={() => { setEditingListField("name"); setListDraft(list.name); }} className="p-1.5 text-gray-600 hover:text-gray-300 rounded transition-colors">✎</button>
            </div>
          )}
          ...
          {editingListField === "description" ? (
            <div className="flex gap-2">
              <textarea
                ...
                value={listDraft}
                onChange={(e) => setListDraft(e.target.value)}
                onKeyDown={(e) => { if (e.key === "Escape") setEditingListField(null); }}
                rows={2}
              ...
              <div className="flex flex-col gap-1">
                <button type="button" onClick={saveListDescription} className="px-3 py-1.5 bg-red-600 hover:bg-red-500 text-white text-xs font-semibold rounded-lg">Save</button>
                <button type="button" onClick={() => setEditingListField(null)} className="px-3 py-1.5 bg-gray-700 hover:bg-gray-600 text-gray-200 text-xs rounded-lg">✕</button>
              </div>
            </div>
          ) : (
            <button
              type="button"
              onClick={() => { setEditingListField("description"); setListDraft(list.description ?? ""); }}
        ...
        <div className="flex items-center justify-between pt-2 border-t border-gray-800">
          ...
          {!adding && (
            <button type="button" onClick={() => { setAdding(true); setBanner(null); }} className="px-3 py-1.5 bg-red-600 hover:bg-red-500 text-white text-sm font-semibold rounded-lg transition-colors">
              + Add Item
            </button>
          )}
        </div>
        ...
        {adding && (
          <div className="bg-gray-900 rounded-xl border border-gray-700 p-4 space-y-3">
            ...
            <div className="flex gap-2">
              <button type="button" onClick={handleAddItem} className="px-4 py-2 bg-red-600 hover:bg-red-500 text-white text-sm font-semibold rounded-lg">Add</button>
              <button type="button" onClick={() => { setAdding(false); setNewItemName(""); setNewItemDesc(""); setAddError(""); }} className="px-4 py-2 bg-gray-700 hover:bg-gray-600 text-gray-200 text-sm rounded-lg">Cancel</button>
            </div>
          </div>
        )}
            ...
            <div key={item.id} className="bg-gray-900 rounded-xl border border-gray-700 p-4">
              ...
              {editingItem?.id === item.id && editingItem.field === "name" ? (
                <div className="flex gap-2 mb-1">
                  ...
                  />
                  <button type="button" onClick={saveItemEdit} className="px-3 py-1.5 bg-red-600 hover:bg-red-500 text-white text-xs font-semibold rounded-lg">Save</button>
                  <button type="button" onClick={() => setEditingItem(null)} className="px-3 py-1.5 bg-gray-700 hover:bg-gray-600 text-gray-200 text-xs rounded-lg">✕</button>
                </div>
              ) : (
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <button type="button" title="Rename" onClick={() => setEditingItem({ id: item.id, field: "name", value: item.name })} className="p-1.5 text-gray-600 hover:text-gray-300 rounded transition-colors">✎</button>
                    <button type="button" title="Delete" onClick={() => setConfirmDeleteItem(item.id)} className="p-1.5 text-gray-600 hover:text-red-400 rounded transition-colors">✕</button>
                  </div>
                </div>
              )}
              ...
              {editingItem?.id === item.id && editingItem.field === "description" ? (
                ...
                <button
                  type="button"
                  onClick={() => setEditingItem({ id: item.id, field: "description", value: item.description ?? "" })}
              ...
              {confirmDeleteItem === item.id && (
                <div className="mt-3 pt-3 border-t border-gray-700 flex items-center gap-3">
                  ...
                  <button type="button" onClick={() => handleDeleteItem(item.id)} className="px-3 py-1 bg-red-700 hover:bg-red-600 text-white text-xs font-semibold rounded-lg">Delete</button>
                  <button type="button" onClick={() => setConfirmDeleteItem(null)} className="px-3 py-1 bg-gray-700 hover:bg-gray-600 text-gray-200 text-xs rounded-lg">Cancel</button>
                </div>
              )}
            </div>
          ))}
